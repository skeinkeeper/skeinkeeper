...
- **Player↔character map (3-way identity)** — Discord user ↔ Foundry user ↔ Foundry actor, so the AI can attribute voice, route Foundry whispers, and address the right character. Holds the Discord user ID (`PII<>`), the Foundry user ID (`PII<>`), the Foundry actor ID, and an optional display name. Both identity columns are erased with the row on per-player deletion.
...
- **Operator setting** — the Discord user ID of the **operator** who receives setup-note DMs, when you designate yourself in the console or via `/skeinkeeper operator claim`. It's the operator's own ID (operator config, not data collected about a player); removed when the campaign is deleted. Alongside it, an optional **operator DM-consent timestamp** records whether you opted in to pause-notification DMs (see below); toggling the consent off deletes the row, and it is likewise removed when the campaign is deleted.
...
- **Player personal information beyond Discord ID, Foundry user ID, and consent records.**
...
**Operator notes go to Foundry GM chat.** When the AI hits a setup problem it can't resolve in-fiction (for example, a player names a character that isn't in the Foundry world), it posts a GM-only Foundry chat message (and whispers the operator's Foundry user when one is known). Players never see these notes. They carry campaign-operational details (a player's display name, a character name) — nothing beyond what's already covered above. The note _messages_ live in Foundry's chat log, not in Skeinkeeper. Discord DMs are used only for the one-time voice-consent prompt, a one-time courtesy redirect if a player DMs the bot after side-channels moved to Foundry, and — **operator-only, opt-in** — the pause notification below.

**Pause-notification DM (operator-only, opt-in).** If the Foundry connection drops mid-session, the session pauses — and the usual operator surface (Foundry GM chat) is exactly what's down. If you, the operator, have opted in (console checkbox or `/skeinkeeper operator dm-consent on`), Skeinkeeper sends you **one Discord DM per pause** saying the session paused and why (the technical cause only — never player content or transcripts). This is a separate, operator-side consent from player voice consent, stored as a timestamp next to your operator designation; turning it off deletes the record. Without the opt-in, the pause is reported only in the web console.
...
Writes an archive (JSON + a readable HTML summary) of everything Skeinkeeper has stored involving that Discord user — their 3-way identity mappings (Discord ID + Foundry user ID + actor), transcript appearances, consent records — into the output directory (`./exports` by default). `--tenant` defaults to `default`.
