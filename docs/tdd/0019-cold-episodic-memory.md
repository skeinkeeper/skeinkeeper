...
PRD refs: 5.1
PRD-rev: 10391ba
ADR constraints: 0002, 0004, 0008, 0010
Author: maintainers
Date: 2026-05-19
Related TDDs: [0008 (LLM provider interface)](./0008-llm-provider-interface.md), [0011 (orchestrator turn loop)](./0011-orchestrator-turn-loop.md), [0013 (dialogue persistence + session lifecycle)](./0013-dialogue-persistence-session-lifecycle.md)
...
- **LanceDB**, one directory per tenant: `<dataDir>/memory/<tenantId>/`.
- Records carry `embedModel` so model-switch is detectable.
- Structured `deltas` field preserved separately from prose in episodic records (ADR-0002).

## Sequencing / implementation plan

Phases map to the numbered sections in Approach: (1) `EmbeddingProvider` interface + local default, (2) `MemoryStore` interface + LanceDB impl, (3) episodic summary generation hook into `endSession`, (4) consolidation pass, (5) retrieval wired into `assembleHotContext`, (6) cold ingestion CLI/web action, (7) erasure adapter.

## Failure modes & edge cases

- Episodic summary generation is best-effort and must not block session teardown; failures are logged, not fatal.
- If a socket/LanceDB error occurs during retrieval, the turn proceeds with the hot context already assembled; memory retrieval failure is non-fatal.
- Model-switch detected via stored `embedModel`; vectors from different models aren't comparable — a migration is required (out of scope for first cut; see Open questions).
...
None — this design directly implements the cold and episodic tiers of ADR-0002's four-tier model and PRD §5.1's memory architecture requirements; no requirement proved infeasible or contradictory.

## Decisions to promote (ADR candidates)

None — the episodic-erasure scoping decision (campaign/tenant-scoped, never player-scoped) is flagged in §7 as warranting its own ADR; that ADR is written alongside the implementation, not here.
