...
Author: maintainers
Date: 2026-05-26
Related TDDs: [0014 (McpFoundryClient)](./0014-mcp-foundry-client.md), [0022 (DM-action coverage audit)](./0022-dm-action-coverage-audit.md), [0026 (side-channels)](./0026-player-dm-side-channels.md), [0027 (MCP bridge gap re-audit + upstream proposal)](./0027-mcp-bridge-gap-reaudit-upstream-proposal.md), [0031 (intake + intake report)](./0031-session-intake-and-intake-report.md), [0032 (autonomous setup actions)](./0032-autonomous-pre-game-setup-actions.md)
...
presence half of §4.8's perception is already shipped in TDD 0023 and is not re-designed
here.
...
`add-actor-items`). Per-audience journal share is a known bridge gap (TDD 0027); v0.5
ships a **Discord side-channel fallback** that delivers the journal content over the
player's existing 1:1 DM thread (TDD 0026's wire) and links to the Foundry entry by name —
behaviorally equivalent from the player's perspective; not equivalent from the
operator-spectator's perspective (the bridge never reveals the entry to the audience). The
fallback is explicitly time-limited: it goes away when the upstream proposal lands.
...
// audience model is TDD 0026's: type Audience = 'table' | `player:${string}` | 'gm'
// v0.5 implementation: Coordinator-table narration for 'table'; Coordinator
// per-player DM (the same wire as the `whisper` tool from TDD 0026) for `player:<id>`;
// no-op (already GM-visible) for 'gm'.
export const share_journal_to_audience = tool({
  name: 'share_journal_to_audience',
  args: {
    journalId: string,
    audience: Audience,              // imported from TDD 0026's audience types
...
2. Calls one or more `McpFoundryClient` methods (or, for the journal-share player path,
   `notify_audience` over the side-channel wire from TDD 0026).
3. Returns the structured result to the orchestrator turn loop; failures bubble as tool-
   call errors and route through `notify_operator` (TDD 0023) on a configurable severity
   threshold.
...
**`share_journal_to_audience`.** Branch on audience tag:

- `'table'` → the Coordinator (TDD 0026 §3) emits a `table`-audience narration that
  includes the journal title + the excerpt (default cap 1500 chars). The same dispatch
  the AI already uses for table-audience narration delivers it to the Discord text
  channel + Foundry chat (existing dialogue-write path). The bridge has no first-class
  "show to all" affordance verified at the time of design (see Open questions); v0.5
  therefore relies on the dialogue-write path rather than a bridge-side `ownership =
OBSERVER` mutation. When live bridge verification or upstream support lands a true
  Foundry-side reveal, this branch additively gains it.
- `player:<id>` → the Coordinator routes through the same per-player DM conversation
  used by the `whisper` tool (TDD 0026 §7). Prefix the message with a framing marker
  (`*A note slips into your hand:*`) + the journal title; excerpt cap default 1500
  chars; suffix `(full entry: <name> — ask your DM to share)`. Other players' DM
  channels are untouched.
- `'gm'` → no-op (the journal is GM-visible by default in Foundry); log + return
  success.

The fallback respects the audience model from TDD 0026 + ADR-0017: per-audience visibility
is preserved (player gets the content; table never sees it). The leak surface is narrower
than the bridge's would be (Foundry can't be relied on to enforce per-player content
visibility yet), and the existing per-audience tests from TDD 0026 cover the channel.
...
Drafted here for the next batch upstream (joining TDD 0027's pending items):
...
This proposal is added to TDD 0027's upstream batch on the next iteration.

### Per-audience journal-share upstream piggyback

The audience-targeted journal share is already in TDD 0027's pending upstream batch ("per-
player content reveal"). This TDD's Discord-side fallback is explicitly the v0.5
implementation; when the upstream lands, `share_journal_to_audience` adds a Foundry-side
branch for `audience: { playerId }` and the player still gets the bridge-side reveal
_plus_ the side-channel narration (the side-channel layer is the "you found a note"
framing; the bridge-side reveal makes the entry visible in the player's Foundry UI).

## Data & state

No new persistent state in Skeinkeeper for these capabilities. State lives in Foundry per
ADR-0018. The Discord-side-channel fallback for player-targeted journal share writes
through TDD 0026's existing dialogue-persistence path; no new tables, no new audience-
tagging schema (the existing `audience` column already handles `player:<id>`).
...
4. `share_journal_to_audience` — Discord side-channel fallback for player audience;
   `notify_table` rendering for table audience; no-op for GM audience.
...
6. Orchestrator wiring of the null stream; orchestrator tool registry adds the four
   tools.
7. Upstream proposal text added to TDD 0027's pending batch.
...
- **Audience-targeted journal share for an audience that includes a non-consented
  player.** TDD 0026's consent gate already covers this; the side-channel wire rejects;
  this tool surfaces the rejection back to the AI.
...
- **Player-targeted journal share larger than Discord's DM character limit.** Excerpt is
  applied; full content link references the Foundry entry by name. The 1500-char default
  is well under Discord's 2000-char limit and leaves room for the prefix marker + suffix.
...
| 4.8 (share journals to audience)            | "share journal entries with a specified audience (`table` / `player:<id>`, per §4.7)"                                                | `share_journal_to_audience` tool with per-audience branching; Discord side-channel fallback for player audience until the upstream lands                                                                  |
...
| 4.8 (Discord voice presence)                | "and to Discord voice presence"                                                                                                      | Already shipped in TDD 0023 (`VoiceIO.presence`); not re-designed here                                                                                                                                    |
...
None. All four tools route through existing `FoundryClient` + `McpFoundryClient` (TDD 0014) and existing Discord side-channel wire (TDD 0026). No new third-party libraries.
...
2. **Per-audience journal share is unfeasible against the current bridge surface.**
   TDD 0027 already named this gap. **Resolution:** v0.5 Discord side-channel fallback;
   upstream piggyback in TDD 0027's batch; tool surface is forward-compatible (the
   Foundry-side branch lights up additively when upstream lands).
3. **Token placement at explicit coords is constrained.** TDD 0022 item 5 named this
   ...
   proposal for a first-class place-at-coords parameter is in TDD 0027's batch
   (already-noted item).
...
| `action.share_journal_to_audience` | `{ campaignId, audienceKind: 'table' \| 'player' \| 'gm', path: 'table-dialogue' \| 'whisper-wire' \| 'gm-noop', success: boolean }` | Journal share attempted, with the path used              |
...
- **Per-audience journal share** inherits TDD 0026's audience invariant + ADR-0017's
  per-audience visibility/erasure model. The side-channel fallback writes the journal
  excerpt to the player's existing 1:1 DM record (already audience-tagged `player:<id>`),
  so erasure of that player's side-channel content also erases the shared excerpt — the
  same property the bridge-side reveal would _not_ automatically give us, making the
  fallback strictly better for erasure semantics at v0.5.
...
5. **Journal share, player audience.** Tool dispatches a per-player DM through the
   Coordinator's `whisper`-style wire (TDD 0026 §7) with the audience-prefixed text;
   other players' channels are untouched (covered by TDD 0026's audience-isolation
   tests).
...
- **Table-audience journal-reveal bridge surface.** The bridge has journal _mutation_
  ...
  bridge. This is a smaller version of the per-player gap (TDD 0027) and folds into the
  same upstream proposal.
- **Combat-tracker control** (start/end/initiative/next-turn) is a known bridge gap
  (TDD 0022 #1; TDD 0027 re-verified). It's not in §4.8's enumerated triggered actions
  but is needed to drive combat. Out of scope for this TDD; tracked in TDD 0022 + TDD
  0027's upstream batch. When the upstream lands, a follow-up TDD adds the combat
  control tools.
