...
PRD refs: 4.2, 5.1
PRD-rev: 10391ba
ADR constraints: 0002, 0003, 0004, 0008, 0011, 0018
Author: maintainers
Date: 2026-05-19
...
> The Foundry-as-source-of-truth decision is recorded as [ADR-0018](../adr/0018-foundry-source-of-truth.md); this TDD retains its design and rationale.
...
- **Chosen:** Foundry MCP bridge (adambdooley default, per ADR-0011) — OSS, self-hostable, covers the MCP surface Skeinkeeper needs.
- **alexivenkov MCP bridge** — rejected per ADR-0011: Patreon-gated, conflicts with project's OSS-first stance.
...
- **Roll20 / Owlbear** — evaluated; rejected: Roll20's character-sheet model is closed and not ruleset-pluggable; Owlbear lacks the per-system data model depth Foundry provides.

## PRD conflicts surfaced (and resolution)

None — this design directly implements ADR-0018 (Foundry as source of truth) and aligns with ADR-0011 (OSS bridge selection); no PRD requirement proved infeasible or contradictory.

## Decisions to promote (ADR candidates)

Promoted to [ADR-0018](../adr/0018-foundry-source-of-truth.md) (Foundry is the authoritative source of truth for mechanical state) during the docs migration.
...
- **Operator without Foundry.** Currently impossible. Skeinkeeper requires Foundry; this design hardens that requirement. If a future "no-VTT-just-Discord-text" mode is wanted, we'd need a `NoopFoundryClient` or a separate orchestrator path. Out of scope for alpha.
