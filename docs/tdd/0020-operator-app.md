...
PRD refs: 4.4, 4.5
PRD-rev: 10391ba
ADR constraints: 0004, 0008, 0009, 0010, 0011
Author: maintainers
Date: 2026-05-19
...
The app's interface surface is the composition root itself: `@skeinkeeper/app` exposes no library API — it's an executable service. Key internal seams:

- `discord.js` `Client` (bot, gateway)
- `DiscordVoiceIO` + `DeepgramStreamingSTT` + `ElevenLabsTTS` + `ElevenLabsVoiceLibrary`
- `AnthropicProvider`, `ToolDispatcher` (`createDefaultRegistry`), `McpFoundryClient` (doc 0014)
- `TenantDb` (persistent, on-disk SQLite)
- Session-manager (start/stop lifecycle, single-process in-process call)
- `ExportService` / `ErasureService` (per-player privacy ops)
...
State mutations flow through the DB-as-bus (§5): UI writes → loop reads each cycle.

## Sequencing / implementation plan

See §8 phasing: 5a (headless), 5b (web UI), 5c (hardening + auth + secrets + docker compose).

## Failure modes & edge cases

- **Multi-session / multi-campaign concurrency**: one active session at a time for alpha; session-manager shape depends on answer (see Open questions).
- **Web auth for remote access**: localhost-only for alpha; remote access requires deliberate auth/TLS story (out of scope).
- **Foundry MCP bridge packaging**: bundled compose service vs. operator-run; ties into INSTALL.md.
...
None — this phase wires existing pieces into the promised operator app; PRD §4.4 and §4.5 requirements are directly satisfied; no requirement proved infeasible or contradictory.

## Decisions to promote (ADR candidates)

None — the "no web framework, Node stdlib only" choice is a design constraint, not a cross-cutting architectural decision requiring an ADR; it's self-contained to this package. The "one process / DB-as-bus" choice is likewise phase-scoped.
