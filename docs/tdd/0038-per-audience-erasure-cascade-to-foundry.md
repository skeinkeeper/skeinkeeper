...
PRD refs: 5.5
PRD-rev: 59a0fda
ADR constraints: 0008, 0010, 0014, 0017, 0018, 0023, 0025
...
- A **`FoundryWhisperDeletionAdapter`** that participates in the existing `ErasureService.erase(scope)` flow with `supportedScopes: ["player"]`. It calls `FoundryClient.deleteChatMessages` (TDD 0037 Band A cap #3) filtered by recipient = the player's Foundry user, removing the player's whisper history from Foundry.
- The **`ErasureReport` shape gains a `partialSuccess` flag and a `manualRemainders` array** to carry per-adapter "your erasure isn't fully complete; here's what's left" information when an adapter cannot fully delete (typically because a bridge capability isn't available at runtime).
...
What this TDD has to get right is **honest reporting when the cascade can't fully complete.** The PRD §5.5 says per-player erasure "deletes both the Skeinkeeper-side dialogue store _and_ the corresponding Foundry whisper history." If the Foundry side fails — bridge unavailable, `delete-chat-messages` cap missing on the running bridge, Foundry temporarily disconnected — Skeinkeeper has three honest options: (a) fail the entire erasure, (b) partial-success with the remainder named, or (c) silently complete Skeinkeeper-side and hide the Foundry remainder. The design-pass decision (recorded in interview) chose (b): **partial-success with explicit "Foundry-side manual cleanup required" remainders in the report, CLI exit non-zero so the operator notices.** This is the more honest of the three options that's still usable; (a) makes erasure unrunnable when the bridge is down (a real operator concern at v0.5), and (c) lies to the operator (and to the player who asked for erasure).
...
export class FoundryWhisperDeletionAdapter implements DeletionAdapter {
  ...
  constructor(
    ...
    private capabilities: BridgeCapabilities, // TDD 0037's probe result
  ) {}
  ...
  async delete(scope: ErasureScope): Promise<DeletionAdapterResult> {
    ...
    // Check bridge capability before the call.
    if (!this.capabilities.has("delete-chat-messages")) {
      return {
        recordsDeleted: 0,
        manualRemainder: {
          reason: "bridge-cap-unavailable",
          foundryUserId,
          message: `The bridge does not expose \`delete-chat-messages\`. Whisper history for Foundry user ${foundryUserId} must be manually deleted via Foundry's GM chat-log UI. Track upstream/fork progress in TDD 0037.`,
        },
      };
    }

    // Cap is available; call it. By-recipient deletes the player's whispers (where they were the recipient);
...
export interface ManualRemainder {
  reason: "no-foundry-user-mapped" | "bridge-cap-unavailable" | "bridge-call-failed" | string;
...
- **Foundry-side:** whisper history for the player's Foundry user, deleted via the bridge's `delete-chat-messages` when available; manual operator action required as a remainder when not.
...
The capabilities object is the same one TDD 0037 builds at session connect; for CLI runs that aren't part of an active session, the CLI runs its own brief capability probe before calling `erase`.
...
4. **`FoundryWhisperDeletionAdapter`** + unit tests (per failure-mode branch: no-mapping, bridge-cap-unavailable, bridge-call-failed, success).
5. **CLI exit-code change** + summary rendering of manual remainders. Unit-tested via the CLI's existing test harness.
6. **CLI capability probe** for `player:delete` runs outside an active session (so a standalone `skeinkeeper player:delete` knows whether the bridge cap is available).
...
- **Bridge cap unavailable at deletion time.** Adapter returns `manualRemainder { reason: "bridge-cap-unavailable" }`. CLI summary names this; exit 2.
- **Bridge cap available but Foundry instance is down.** `deleteChatMessages` rejects with a transport error. Adapter returns `manualRemainder { reason: "bridge-call-failed" }`. CLI summary names this; exit 2; operator retries when Foundry is back.
...
- **`delete-chat-messages` deletes more than expected** (e.g., the bridge implementation interprets `by-recipient` as "messages where the user is _any_ recipient" including group whispers). The integration test against a real Foundry validates the exact semantic; if the bridge cap's semantic doesn't match the expected one, the cascade is over-deleting OR under-deleting. We use Skeinkeeper's deletion-log + Foundry's native chat-log persistence to spot mismatches.
...
- **Cascade — bridge cap unavailable.** _Observation point:_ unit test — capabilities.has("delete-chat-messages") = false. _Expected:_ no bridge call recorded; `{ recordsDeleted: 0, manualRemainder: { reason: "bridge-cap-unavailable" } }`; `partialSuccess: true`.
- **Cascade — bridge call fails.** _Observation point:_ unit test — fake throws on `deleteChatMessages`. _Expected:_ `{ recordsDeleted: 0, manualRemainder: { reason: "bridge-call-failed" } }`; `partialSuccess: true`; the error message is in the manual-remainder message string.
...
- **`deletion_log` records partial-success.** _Observable surface:_ the `deletion_log` table after a cascade. _Observation point:_ integration test — run erasure with a bridge-cap-unavailable cascade; query the `deletion_log` row. _Expected:_ `partial_success = 1`; `manual_remainders` is a JSON array with one entry.
...
- **Live: end-to-end cascade against real Foundry + bridge with Band A cap #3 available.** Operator creates a campaign, exchanges a few whispers between the AI DM Foundry user and a player's Foundry user, runs `skeinkeeper player:delete`; observes (in Foundry GM chat log) that the player's whispers are gone; report exits 0. Operator-validated.
...
- `FoundryClient.deleteChatMessages` from TDD 0037 Band A cap #3 (v0.5-blocking).
- The 3-way identity map from TDD 0036.
- The capability probe from TDD 0037.
...
- **Fail erasure entirely when bridge cap is unavailable** (option (a) in §Approach). Rejected: makes erasure unrunnable when the bridge is down — a real operator concern given v0.5 ships only after Band A but live conditions vary. The bridge can go down mid-campaign; players' erasure requests don't pause for bridge availability.
...
- **Queue Foundry-side deletions for retry when the bridge recovers.** Considered (a "background re-cascade" worker that retries `bridge-call-failed` manualRemainders later). Rejected for v0.5 scope: adds a stateful worker for an operator-action that's already an operator-controlled retry (`skeinkeeper player:delete --subject ... --yes` is idempotent — re-runs are safe). Revisit if operator demand emerges.
- **A `FoundryWhisperExportAdapter` symmetric to the deletion adapter.** Considered. Rejected for v0.5: Foundry's chat log is exportable via Foundry's own GM tooling (a stable surface that doesn't depend on the bridge); duplicating it in Skeinkeeper would be a maintenance liability. The HTML summary points the operator at Foundry's tool.
...
3. **`campaign:delete` and `tenant:delete` scopes don't have a Foundry-side analog.** Foundry doesn't have a "delete this campaign's data" surface that's tractable from the bridge. **Resolution:** documented as a known limitation in PRIVACY.md ("Skeinkeeper-side campaign deletion doesn't remove Foundry-side chat history; operator deletes the Foundry world or uses Foundry's own GM tools").
...
- **Background re-cascade for `bridge-call-failed` remainders.** Deferred per §"Dependencies considered → Alternatives." Revisit if operator demand emerges; v0.5 ships with manual retry.
