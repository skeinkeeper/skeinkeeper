...
- **Default-off is the failure-safe state.** With no env vars, both clients are no-ops (`noopAnalytics()` / `noopCrash()`); a missing key/DSN also yields a no-op even if `enabled` is set.
- **Strict env gate.** Only the literal `"1"` enables a stream; typos (`"true"`, `"0"`, empty) default to disabled, so a misconfiguration never accidentally phones home.
...
- **Chosen:** `posthog-node` (analytics sink) and `@sentry/node` (crash sink) — the two sinks ADR-0009 names; both OSS client SDKs, constructed only when their stream is enabled.
...
None — this design directly implements ADR-0009 (telemetry opt-in) and ADR-0010 (privacy as architecture); no PRD requirement proved infeasible or contradictory.

## Decisions to promote (ADR candidates)

None — the durable cross-cutting decisions here are already captured: opt-in-only telemetry and the two-stream/single-emission-surface model in ADR-0009, and PII handling in ADR-0010.

## Alternatives considered

(Design alternatives weighed beyond the dependency choice above.)
