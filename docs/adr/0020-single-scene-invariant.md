...
> The _design_ of side-channels (the Coordinator/concurrency model, the audience data model,
...
An AI DM connected over Discord has no "one mouth, one attention budget" limit, so it _can_
...
- Foundry exposes a single active scene and already personalizes _within_ it (per-player
