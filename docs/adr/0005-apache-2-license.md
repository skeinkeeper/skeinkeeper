...
- **Permissive enough to allow third-party forks, integrations, and derivative works** without the friction GPL introduces. This matches the project's goal of broad community adoption. Apache 2.0 _permits_ downstream commercial use by third parties; it does not commit Skeinkeeper itself to any particular distribution model — the project is and remains self-hosted, open-source software.
