...
The orchestrator enforces this at the engine level: any model output that _appears_ to assert a state change but does not call a tool is flagged. Initially we surface these as warnings in the audit log; eventually we may classify and reject them.
