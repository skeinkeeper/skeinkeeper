...
2. **The AI must have a tool for every in-play DM action.** Where the VTT (Foundry) supports an action but our MCP bridge does not expose it, that is a coverage gap to close (wire it, contribute it upstream, or fork the bridge). See the DM-action coverage audit ([design doc 0022](../design/0022-dm-action-coverage-audit.md)).
...
**Negative / accepted**
- Some in-play DM actions are blocked today by the bridge's tool surface (e.g., direct HP changes, GM-side dice rolls, combat-tracker control). Under this ADR those are **bugs to close**, which raises the priority of bridge contributions or a fork (doc 0022).
