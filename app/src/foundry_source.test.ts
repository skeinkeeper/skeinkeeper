...
import { loadOrCreatePairingSecret, resolvePairingSecret } from "./foundry_source.js";
...
describe("loadOrCreatePairingSecret", () => {
  ...
  it("reports `created` only on the boot that generated the secret", () => {
    // The caller prints the plaintext only when `created` is true, so that a
    // credential is not re-emitted into container logs on every restart.
    const dir = freshDir();
    const first = resolvePairingSecret(dir, "");
    expect(first.created).toBe(true);

    const second = resolvePairingSecret(dir, "");
    expect(second.created).toBe(false);
    expect(second.secret).toBe(first.secret);

    // An operator-supplied secret was never generated by us, so it is not "created"
    // either — nothing to print, nothing persisted.
    expect(resolvePairingSecret(freshDir(), "operator-set")).toEqual({
      secret: "operator-set",
      created: false,
    });
  });
