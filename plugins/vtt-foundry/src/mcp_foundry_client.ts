...
function addonOnly(method: string): never {
  throw new Error(
    `McpFoundryClient.${method}: not available on the withdrawn third-party connector. TDD 0041 implements this on the first-party add-on.`,
  );
}

/**
 * Legacy FoundryClient over the withdrawn third-party MCP connector
 * (TDD 0014, superseded by TDD 0041). Reads that already existed on the
 * 0014 surface still map to that connector. Table-text and token-spawn
 * writes that the connector never had throw; MockFoundryClient is the
 * test seam and ModuleFoundryClient is the production replacement.
 */
export class McpFoundryClient implements FoundryClient {
  ...
  async postChatMessage(_args: PostChatMessageArgs): Promise<{ messageId: string }> {
    throw addonOnly("postChatMessage");
  }

  async deleteChatMessages(_args: DeleteChatMessagesArgs): Promise<{ deletedCount: number }> {
    throw addonOnly("deleteChatMessages");
  }

  subscribeChatEvents(_handler: (event: FoundryChatEvent) => void): () => void {
    return () => {};
  }
